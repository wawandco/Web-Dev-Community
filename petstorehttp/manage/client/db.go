...
import (
	"errors"
	idb "petstore/manage/internal/db"
...
// clients table database. A nil tx passed as argument
// will use a default internal db connection.
func DB(tx *pop.Connection) Database {
	if tx == nil {
		return &db{idb.Connection}
	}
	return &db{tx}
}
